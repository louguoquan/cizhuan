...
+(id)getXuanCheListDataWithType:(NSInteger)type CompletionHandler:(void (^)(XuanCheDataModel *, NSError *))completionHandler
{
    ...
    return [self get:path completionHandler:^(id responseObj, NSError *error) {
        !completionHandler ?: completionHandler ([XuanCheDataModel parse:responseObj],error);
    }];
}

+(id)getBrandsDetailWithValue:(NSString *)value CompletionHandler:(void (^)(BrandsDetailModel *, NSError *))completionHandler
{
    NSString *path = [NSString stringWithFormat:kBrandsDetailPath,value];
    return [self get:path completionHandler:^(id responseObj, NSError *error) {
        !completionHandler ?: completionHandler ([BrandsDetailModel parse:responseObj],error);
    }];
}
...
{
    advertModel *model1 = [[advertModel alloc]init];
    model1.seriesname = @"力帆迈威";
    model1.img = @"http://adm3.autoimg.cn/admdfs/g13/M04/84/A8/wKjBylgPGQCAB7nNAACIEjtXKmk513.png";
    
    advertModel *model2 = [[advertModel alloc]init];
    model2.seriesname = @"传祺GS4";
    model2.img = @"http://img2.autoimg.cn/admdfs/g6/M03/9D/B5/wKgH3FgqjgiAbTsZAABE7SJlH_c696.png";
    
    advertModel *model3 = [[advertModel alloc]init];
    model3.seriesname = @"欧蓝德";
    model3.img = @"http://img2.autoimg.cn/admdfs/g21/M13/71/18/wKgFWlgcRdKAYTo4AACQIJnBqJE749.png";
