...
@interface JJRegistViewController ()
...
@implementation JJRegistViewController
...
- (void)setUpNav
{
    ...
    _mobileCell.ys_textFiled.placeholder = @"手机号码";
    ...
    img.image = [UIImage imageNamed:@"+86"];
    
    UIImageView *imgspace = [[UIImageView alloc]initWithFrame:CGRectMake(0, 0, 65, 35)];
    [imgspace addSubview:img];
    
    _mobileCell.ys_textFiled.leftView =imgspace;
    _mobileCell.ys_textFiled.leftViewMode = UITextFieldViewModeAlways;
    ...
    if (self.isResetPwd) {
        ...
        self.loginWithOther.text = @"邮箱找回";
        
    }else{
        
        self.loginWithOther.text = @"邮箱注册";
    }
...
- (void)taptoEmail:(UITapGestureRecognizer *)tap{
    ...
    JJRegistWithEmailViewController *vc = [[JJRegistWithEmailViewController alloc]init];
...
- (void)nextStep:(UIButton *)btn{
    ...
    if (self.isResetPwd) {
        ...
        //发送手机验证码
        [JJLoginService requestMobileCodeSend:_mobileCell.ys_text type:@"1" category:@"1" Completion:^(id result, id error) {
        ...
        [MBProgressHUD showLoadingToContainer:[UIApplication sharedApplication].keyWindow];
        //发送手机验证码
        [JJLoginService requestMobileCodeSend:_mobileCell.ys_text type:@"0" category:@"1" Completion:^(id result, id error) {
...
- (UILabel *)loginTitle
{
    if (!_loginTitle) {
        ...
        if (self.isResetPwd) {
            ...
            _loginTitle.text = @"手机找回";
            
        }else{
            _loginTitle.text = @"手机注册";
