...
@interface JJCheckOutViewController ()
...
@implementation JJCheckOutViewController
...
- (void)query{
    ...
    [JJShopService JJMobileGiftGiftListCompletion:^(id result, id error) {
        if (self.page == 1) {
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.dataArray.count) {
        JJGiftModel *model = self.dataArray[indexPath.row];
        cell.type = @"2";
        cell.giftModel = model;
        cell.JJCardListCellClick = ^() {
            
            MMAlertView *alertView = [[MMAlertView alloc] initWithTitle:@"温馨提示" detail:@"是否确认解锁" items:@[MMItemMake(@"取消", MMItemTypeNormal, nil), MMItemMake(@"确定", MMItemTypeHighlight, ^(NSInteger index) {
                
                [MBProgressHUD showLoadingText:@"解绑中" toContainer:self.view];
                [JJShopService JJMobileGiftUnlock:model.ID Completion:^(id result, id error) {
