...
@interface JYAsyncSocket()<GCDAsyncSocketDelegate>
//定时器
...
- (BOOL) connecteServerWith:(NSString *)host  onPort:(uint16_t)port{
    static BOOL success;
    GCDAsyncSocket *sockte =  [JYAsyncSocket shareAsncSocket].socket;
...
- (void)socket:(GCDAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag{
    ...
    self.lenght = [JYAsyncSocket unpackingLenght:msg];
    ...
    if (self.completeData.length >= self.lenght) {
        NSString *type = [JYAsyncSocket unpackingHeard:msg];
        if (_reciveBlock ) {
            //            NSLog(@"tmsg = %@ ", msg);
